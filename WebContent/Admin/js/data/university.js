...
app.controller('universitycontroller', function($scope, $http, $timeout) {
	$http.get('../listUniversity').success(function(data) {
	...
	$scope.editUser = function(id) {
		...
		if (id == 'new') {
			...
			$('#subcat').select2().select2('val', $scope.list[x].short_name_id);			
			setSelectedValue(status, $scope.list[x].status1);
			$("#code").val($scope.list[x].category_id);
		}
	};
	$scope.updateStatusUniversity = function(e) {
		var data = {
			'id' : e
		};
		$.post("../updateStatusUniversity", data).success(function(data, status, headers) {
			if (data == "Success") {				
				$scope.loadData();
			}
		});
