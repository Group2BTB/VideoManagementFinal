...
<body class="page-body page-fade" data-url="#">
	...
	<div class="page-container">
		...
		<div class="main-content">
			...
			<div class="clearfix"></div> -->
					<div class="panel panel-primary">
						...
						<div class="panel-heading">
							<i class="glyphicon glyphicon-comment"></i> List Commment
						</div>
						...
						<table class="table table-bordered" ng-show="filteredItems > 0">
							<tr class="active" style="cursor: pointer;">
								<th class="center">No&nbsp;<a
									ng-click="sort_by('comment_id');"><i
										class="glyphicon glyphicon-sort"></i></a></th>
								<th class="center">Comment&nbsp;<a
									ng-click="sort_by('description');"><i
										class="glyphicon glyphicon-sort"></i></a></th>
								<th class="center">Comment By&nbsp;<a
									ng-click="sort_by('username');"><i
										class="glyphicon glyphicon-sort"></i></a></th>
								<th class="center">Action</th>
							</tr>
							<tr ng-click="updateView(d.comment_id)" ng-show="!d.view"
								class="active"
								...
								<td>{{d.description | limitTo:25}}...</td>
								<td class="center">{{d.username}}</td>
								<td class="center">
									<button type='button' ng-click="viewComment(d.comment_id)"
										class='btn btn-info icon_margin_top'>
										<span class='glyphicon glyphicon-eye-open'></span>
									</button>
									<button type='button' ng-click="editUser(d.comment_id)"
										class='btn btn-danger icon_margin_top'>
										<span class='glyphicon glyphicon-remove'></span>
									</button>
								</td>
							</tr>
							<tr ng-show="d.view" class=""
								ng-repeat="d in filtered = (list | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit">
								<td class="center">{{($index+1)}}</td>
								<td>{{d.description | limitTo:25}}...</td>
								<td class="center">{{d.username}}</td>
								<td class="center">
									...
									<button type='button' ng-click="deleteComment(d.comment_id)"
										class='btn btn-danger icon_margin_top'>
										<span class='glyphicon glyphicon-remove'></span>
									</button>
								</td>
			...
			<!-- End More Filter -->
			<!-- Start More Filter -->
			<div id="viewCom" class="modal fade bs-example-modal-lg">
				<div class="modal-dialog">
					<div class="modal-content" style="margin: 0 auto;">
						<form method="post" action="" id="frmDepartment">
							<div class="modal-header" style="text-align: center;">
							...
							<div class="modal-body">
								<div class="panel panel-default">
									...
									<!-- List group -->
									<ul class="list-group">
										<li class="list-group-item">Date: <span id="cdate"></span></li>
										<li class="list-group-item">User: <span id="cuser"></span></li>
										<li class="list-group-item">Video: <span id="cvideo"></span></li>
										<li class="list-group-item">View: <span id="cview"></span></li>
										<li class="list-group-item"><div class="clearfix"></div>
											<span style="float:left">Like: <span id="clike"></span></span>     
											<span style="float:right">Unlike: <span id="cunlike"></span></span>
											<div class="clearfix"></div>
										</li>
									</ul>
								</div>								
		...
		<script>
			...
			$("document").ready(function(e) {				
				...
				$("#viewCom").on("show", function() { // wire up the OK button to dismiss the modal when shown
					$("#viewCom a.btn").on("click", function(e) {
						console.log("button pressed"); // just as an example...
						$("#viewCom").modal('hide'); // dismiss the dialog
					});
				});

				$("#viewCom").on("hide", function() { // remove the event listeners when the dialog is dismissed

					$("#viewCom a.btn").off("click");
				});

				$("#viewCom").on("hidden", function() { // remove the actual elements from the DOM when fully hidden
